...
function mountScrollWorld(container, config) {
  ...
  if (config.brand) {
    ...
    if (config.brand.logo) {
      const lg = el('img', 'sw-brand__logo'); lg.src = config.brand.logo; lg.alt = ''; brand.appendChild(lg);
    } else {
  ...
  function read() {
    ...
    const chromeF = smooth(1 - Math.max(0, y - totalW * vh) / (0.6 * vh));
    ...
    for (let i = 0; i < N; i++) {
      ...
      else if (i === N - 1) cop = before ? 0 : smooth(pr / 0.4) * chromeF; // holds CTA, then yields to page
    ...
    route.style.opacity = chromeF;
    route.style.pointerEvents = chromeF > 0.5 ? 'auto' : 'none';
    scrollbar.style.opacity = chromeF;
    nav.style.opacity = chromeF;
    nav.style.pointerEvents = chromeF > 0.5 ? 'auto' : 'none';
